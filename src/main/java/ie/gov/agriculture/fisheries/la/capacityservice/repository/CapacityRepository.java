...
public interface CapacityRepository extends CrudRepository<Capacity, String> {
	@Query (
		value = "" +
			...
			"COALESCE((SELECT B.GROSSBALANCE FROM capaccountsegment B WHERE B.capaccountid = A.capaccountid AND B.UOMID = 91359 and B.segmentid = A.segmentid), 0) gt, " + 
			"COALESCE(A.kw, 0) kw, A.vesselid, A.proposed /*, A.segmentid, A.CREATEDATE*/ FROM ( " + 
			...
			"    else '0' " + 
			"end fleetSegment, " + 
			"case when (b.segmentid is null) then 0 else b.segmentid end fleetSubSegment, a.vesselid, " + 
			...
			"(select GROSSBALANCE from capaccountsegment where CAPACCOUNTID = a.capaccountid and CAPSEGMENTID = b.capsegmentid and UOMID = 91290) kw, " + 
			"a.CREATEDATE, ROW_NUMBER() OVER (partition by a.capaccountid ORDER BY a.capaccountid) CapAccRank " + 
