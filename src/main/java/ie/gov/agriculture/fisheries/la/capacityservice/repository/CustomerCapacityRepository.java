...
public interface CustomerCapacityRepository extends CrudRepository<CustomerCapacity, String> {
	...
	@Query (
		value = "" +
			...
			"(SELECT B.GROSSBALANCE FROM capaccountsegment B WHERE B.capaccountid = A.capaccountid AND B.UOMID = 91359 and B.segmentid = A.segmentid) gt, " + 
			"A.kw, A.vesselid, A.proposed /*, A.segmentid, A.CREATEDATE*/ FROM ( " + 
